# PawnSim — RimWorld Fidelity Audit (2026-05-29)

Scope: read-only audit of `unity-project/Assets/Scripts/` gameplay constants vs RimWorld
...
`GameClock.cs:17`). So when RimWorld states "per day," the prototype-equivalent
real-second rate at 1× speed is `RimWorldPerDay / 240`. Decay rates in
`PawnNeeds.cs` are expressed in **units (0–100) per real second**, so to be
RimWorld-faithful a need that should empty over N in-game days must decay at
`100 / (N × 240)` units/sec.
...
- **HIGH** — breaks the RimWorld game feel or is wildly off (order-of-magnitude error;
...
that are wrong in a way that hurts the RimWorld feel.

---

## 1. Discrepancy table

| System | Prototype value (file:line) | RimWorld value (source) | Severity | Suggested fix |
|---|---|---|---|---|
...
| **Mood-break threshold** | single `moodBreakThreshold = 20`, recover at 35 (`PawnNeeds.cs:37-38`) | Three tiers: **minor 35%, major 20%, extreme 5%**; break is a mean-time-to-event below each, not instant ([Mental Break Threshold]) | **MED** | Prototype collapses 3 tiers to 1. The single threshold ≈ RimWorld's "major" (20%). Fidelity win: rename/treat 20 as "major," optionally add a 35 minor. Low effort, but logic-ish — flag for operator. |
| **Skill XP curve** | `XPToLevel(L) = 100·L^1.5` → L0→1 = 100 XP, L1→2 = 283, L2→3 = 520 (`PawnSkills.cs:66`) | RimWorld: **1000 XP per level flat up to ~10**, rising after; ~4000 XP/day soft cap ([Skills]) | **MED** | Early levels come ~10× too cheap (100 vs 1000). Multiply base from 100→~`1000` (and/or flatten the exponent) so early skill gains aren't trivial. |
| **Work speed per level** | `+0.04 (4%) per level`, linear (`PawnChopper.cs:131`, `PawnBuilder.cs:116`, `PawnGatherer.cs:109`, `PawnMiner.cs:98`) | Construction/Mining **+~15%/level** (≈×3 at lvl20 from a low base); cooking ~11%/level ([Skills]) | LOW–MED | 4%/level (×1.8 at lvl20) is flatter than RimWorld's curve but a defensible prototype choice. Bump to ~0.06–0.08 if a stronger skill-progression feel is wanted. |
...
| **Research bench rate** | `pointsPerSecondPerBench = 2`/sec × researcher speed sum (`ResearchManager.cs:43`) | 0.00825 pts/tick × 60 = **~0.5 pts/sec** at baseline; Intellectual +11.5%/level ([Research Speed]) | LOW | Prototype is ~4× faster, but tech costs are also tiny (100–300 vs RimWorld 100–4000), so *time-to-complete* feels fine. Leave; it's a coherent prototype scaling. |
...
| **Rice/crop grow time** | `growthPerSecond = 0.011` → ripen in ~90s = **~0.38 in-game day** (`CropEntity.cs:22`) | Rice **3 base days (5.54 real-grow days)** to mature ([Rice plant]) | LOW | Deliberately accelerated for session pacing (comment cites the tradeoff). ~8–14× faster than RimWorld but a justified prototype choice. Leave. |
| **Crop yield** | `harvestFood = 8` per harvest (`CropEntity.cs:24`) | Rice yields **6 units/plant** ([Rice plant]) | LOW | Close enough (8 vs 6); fine. |
| **Wolf HP** | `maxHp = 18` (`WolfEnemy.cs:12`) | Timber wolf body size ~0.85, **~50 HP-equivalent** health pool | **MED** | Wolf dies in ~4 arrows; RimWorld wolves are tankier. But scaled to the prototype's small dmg numbers it's coherent. Bump to ~30 if wolves feel like paper; else low. |
...
| **Wolf move speed** | chase `2.5`, wander `0.8` (`WolfEnemy.cs:17-18`) | Timber wolf move **~4.6 c/s** ([Timber wolf]) | LOW | Slower than RimWorld; if pawn speed is bumped to 4.6, wolves should also rise (~3.5–4.6) or they can never catch a pawn. **Tie to the pawn-move-speed fix.** |
...
| **Storyteller threat tiers** | Cassandra day 3/7/14, Phoebe 6/14/25, raids every 3 days at 06:00 (`AIDirector.cs:59-76,116-119`) | Storytellers use **threat points** scaling with wealth+time, not fixed day gates ([no clean wiki value; knowledge]) | LOW | Day-gated tiers are a sound prototype abstraction of the points system. Not a wrong "value." |
...
4. **Bow/arrow damage consistency — `ResearchManager.cs:67` description vs `ArrowProjectile.cs:16`.**
   ...
   toward ~12 (closer to RimWorld's 18) **or** correct the description string. This is a
   truth-in-UI bug, not just tuning.

5. **Skill XP base — `PawnSkills.cs:66`  `100f * L^1.5` → `1000f * ...`** (or flatten toward
   RimWorld's flat ~1000/level). Early levels currently cost 100/283/520 XP — roughly 10× too
...
- **Recreation/Joy need** — RimWorld's 4th core need is absent; prototype has only Food/Sleep/Mood
...
## 4. Uncertain RimWorld values (operator should decide)
...
- **Mood baseline** — RimWorld mood is a thought-sum with no single "decay rate," so any
...
- [Move Speed](https://rimworldwiki.com/wiki/Move_Speed) — human base 4.6 c/s
- [Mental Break Threshold](https://rimworldwiki.com/wiki/Mental_Break_Threshold) — minor 35% / major 20% / extreme 5%
- [Skills](https://rimworldwiki.com/wiki/Skills) — ~1000 XP/level, 4000/day soft cap, +15%/lvl construction/mining
- [Saturation](https://rimworldwiki.com/wiki/Saturation) / [Needs](https://rimworldwiki.com/wiki/Needs) — hunger 1.6 nutrition/day; work thresholds rec 35% / eat 30% / rest 30%
- [Rest](https://rimworldwiki.com/wiki/Rest) / [Sleep Fall Rate](https://rimworldwiki.com/wiki/Sleep_Fall_Rate) — 10.5 h to full rest
- [Mood](https://rimworldwiki.com/wiki/Mood) — mood is a sum of thought offsets
- [Short bow](https://rimworldwiki.com/wiki/Short_bow) — 18 dmg, 12-tile range
- [Research Speed](https://rimworldwiki.com/wiki/Research_Speed) — 0.00825 pts/tick base, +11.5%/Intellectual lvl
- [Research](https://rimworldwiki.com/wiki/Research) — project costs 100–4000
- [Rice plant](https://rimworldwiki.com/wiki/Rice_plant) — 3 base / 5.54 grow days, yield 6
- [Timber wolf](https://rimworldwiki.com/wiki/Timber_wolf) — bite 12 dmg / 2s, 18% AP
- [Table of Human Body Parts](https://rimworldwiki.com/wiki/Table_of_Human_Body_Parts) / [Body Parts](https://rimworldwiki.com/wiki/Body_Parts) — torso 40 HP
- [Weapons](https://rimworldwiki.com/wiki/Weapons) — fist ~8.2, melee 9–25
- [Stats](https://rimworld.fandom.com/wiki/Stats) — carrying capacity 75 kg
- [Health](https://rimworldwiki.com/wiki/Health) — downing from pain/blood-loss/consciousness
