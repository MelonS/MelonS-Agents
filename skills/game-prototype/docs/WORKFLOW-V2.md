...
스킬의 성공 지표 = **2번째 게임이 같은 품질에 도달하는 속도**.

### 7. 효과 어서션 — "클릭 PASS"는 효과의 증거가 아니다 (2026-06-12, 운영자 승인 루프 개정)
하네스 스텝 PASS 는 **입력이 전달됐다**까지만 보증한다.  셋업 클릭(메뉴/월드) 뒤에는
반드시 효과 probe 로 **상태 변화**를 어서트한다 — `growZoneCells`/`mineMarks`/
`roofCells`/`blueprintCount`/`structureCount`/`cropCount`/`resourceAtLeast`/
`chopDesignations`.  근거 사건(증류): 지시 메뉴의 글리프 매칭 사각 + designation
7종의 raw Input 사각이 겹쳐 **모든 자율 소크에서 벌목/채광/경작/지붕 지정이 무음
무효**였는데, '클릭 PASS'만 보고 수십 런을 유효로 오판했다.
→ 일반 규칙: **하네스 PASS 는 입력 전달까지만 보증한다 — 효과는 별도 probe 로.**

### 8. 루브릭 + 격리 채점 (Ralph loop — Lance Martin 'Designing loops with Fable 5' 적용)
"루브릭이 모델보다 더 많은 일을 한다.  쓰레기 루브릭 + 좋은 모델 = 자신 있게 틀리는
루프."  소크/플레이테스트/시각 검수는:
- **(a) 루브릭 먼저**: 작업 시작 전에 Done 정의를 루브릭 문서로 고정
  (`docs/rubric-*.md`).  기능 항목은 기계 어서션(카운트/로그/좌표)으로,
  LLM 주관 판정은 시각 품질 등 진짜 주관 영역에만.  **전제 검증(셋업 스텝 그린,
  OVERALL)이 0순위** — 깨진 전제 위의 측정은 무효이며, 루브릭은 '빨리 죽는 순서'로
  배열한다 (1사이클 30분짜리 루프에서 낭비 차단).
- **(b) 격리 채점**: 판정은 **작업 맥락이 없는 grader 서브에이전트**가 수행한다
  (자기채점 편향 차단 — 작업자가 OVERALL 미확인 채 결론을 보고한 실사건).
  증거는 큐레이션 금지: 샷 디렉토리 전체 + 원본 로그 경로를 통째로 넘긴다.
  grader 는 판정·증거 인용만, 수정 제안 금지.  작업자는 평결을 인용해 보고한다.
- **(c) 루프**: grader F 항목별 피드백 → 수정 → 재실행, 그린까지.  소크의
  "N일 생존" 류 결론은 OVERALL 그린 + grader 평결이 있을 때만 보고 가능.

### 9. 실패 증류 (fail → investigate → verify → distill → consult)
규칙 6의 강화: 실패 1건마다 "왜"를 검증된 진단으로 좁히고, **다음에도 적용되는
일반 규칙 1줄**로 증류해 함정 목록(이 문서/메모리/playbook)에 기록한다.  다음 작업
시작 시 함정 목록을 consult — 같은 실패를 재유도하지 않는다.  (기존 '함정 명문화'
관행의 정식화 — bash cwd 절대경로, 직렬화 기본값, Sprite.Create readable 등이 선례.)
...
5. **2번째 게임으로 스킬 검증** — playbook 만 보고 2번째 게임 진행, 막히는 지점이
   곧 playbook 의 구멍.
...
- **시나리오 작성 함정 6번 (2026-06-12)**: worldclick 은 반드시 셀 중심(.5) 좌표로 —
  정수 경계 좌표는 카메라 재투영 왕복의 부동소수 오차로 ±1 셀 스냅된다 (ㅁ자 벽
  미폐합·문/벽 동일셀 중첩·경작 6→3 의 공통 원인, grader 2차 적발).
- **시나리오 작성 함정 7번 (2026-06-12)**: 월드 오브젝트(나무·광맥·베리) 클릭 좌표는
  추측·스샷 픽셀 실측 금지 — **씬 파일이 정답**. 맵은 결정론(seed 고정)이고 엔티티
  이름이 셀 좌표를 인코딩한다 (`grep -o "Tree_[A-Za-z]*_[-0-9]*_[-0-9]*" Game.unity`,
  StoneVein_x_y 동일). 셀 (x,y) → 클릭 (x+.5, y+.5). 소크 r2 의 '벌목 2/3 빗나감
  (at=empty) → 목재 경제 8게임일 미검증'의 원인이자, 쇼케이스 픽셀 실측 5/6 빗나감의
  재발 원인.
- **시나리오 작성 함정 8번 (2026-06-12)**: 일시정지(speed 0) 상태에서 scaled 대기는
  영원히 안 끝난다 — 하네스가 wait(정지 시 realtime 폴백)·shot flush·clickui 정착을
  realtime 으로 처리하도록 수정됨(60913fe). 정지 비트(SPD 0 → SHOT → wait → SPD 1)는
  이제 지원 패턴. 장시간 시나리오는 JSON `timeoutSec` 명시 필수(러너 기본 240s).
- **시나리오 작성 함정 9번 (2026-06-12)**: 소모형 지정(벌목 등)의 어서션은 지정 직후에 —
  지정이 작업으로 소비된 뒤(몇 게임분 후)에 검사하면 designations=0 거짓 FAIL.
