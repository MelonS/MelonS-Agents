...
namespace MelonS.GameProto
{
    ...
    public class WallEntity : MonoBehaviour
    {
        ...
        private void Awake()
        {
            // 벽에는 그림자를 붙이지 않는다 (2026-07-31 실측으로 되돌림).
            //  벽은 서로 맞붙어 있어서 **개별 실루엣 그림자가 서로 겹친다** —
            //  집 한 채가 사각 그림자 수십 개의 지저분한 덩어리가 됐다.
            //  건물 그림자를 제대로 하려면 방 단위 외곽선을 뽑아 하나로 눕혀야 하는데,
            //  그건 별도 작업이다.  지금은 **나무처럼 홀로 선 것**만 그림자를 갖는다 —
            //  그것만으로도 아침/정오 차이가 화면에서 읽힌다(실측 확인).
            if (hp <= 0f) hp = maxHp;
        }
