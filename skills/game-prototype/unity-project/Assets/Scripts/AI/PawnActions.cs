...
namespace MelonS.GameProto.AI
{
    ...
    public class GoSleepAction : IPawnAction
    {
        ...
        internal static Vector2 BedStandPos(BedEntity bed, Vector2 from)
        {
            ...
            //  밖으로 나가는 그림이 된다.
            //
            //  값이 0.5 가 아니라 0.25 인 이유: `CellToWorld` 는 칸 **중심**이므로
            //  +0.5 는 계산상 정확히 반 칸인데, 주민 스프라이트의 피벗이 **발밑**이라
            //  화면에서는 몸통이 한 칸 통째로 올라간 것처럼 읽힌다 — 운영자 확인
            //  "반칸올리라고 했는데 한칸 올라갔네".  보이는 것이 기준이므로 절반으로.
            //  1×1 잠자리는 칸이 하나뿐이라 그대로 둔다 (올리면 잠자리 밖으로 나간다).
            if (bed.Size.y >= 2) pos.y += 0.25f;
            return pos;
        }
    }
