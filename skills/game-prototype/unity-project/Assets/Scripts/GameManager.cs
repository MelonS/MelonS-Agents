...
namespace MelonS.GameProto
{
    ...
    public class GameManager : MonoBehaviour
    {
        ...
        // 시작 콜로니스트 배치의 **코드 정본** (2026-08-01).
        //
        // 왜 코드로 되박나: spawnPositions 는 [SerializeField] 라 **씬에 저장된 값이
        // 코드 기본값을 덮는다**.  오늘만 statusGap·튜토리얼 팁에서 같은 함정을 두 번
        // 겪었다.  씬 재베이크 없이 코드가 단일 정본이 되게 한다.
        //
        // 3인 → 6인으로 늘리는 이유: 시연 영상이 12/100 에 머물렀고 이유는 "머하는
        // 게임인지 모르겠음" 이었다.  화면에 사람 3명·집 1채뿐이면 장르를 아는 사람이
        // 봐도 '축소판 프로토타입' 으로 읽힌다.  콜로니 심은 **여러 사람이 동시에 각자
        // 다른 일을 하는 것**이 장르의 얼굴인데, 3명으로는 그 그림이 안 나온다.
        // 6인이면 벌목·운반·요리·연구·건축이 한 화면에서 동시에 보인다.
        //
        // 배치는 스폰 지점 주변에 흩뿌린다 — 일렬이면 같은 목표를 골라 겹쳐 움직인다
        // (운영자 fb '림들 겹쳐서 이동').
        private static readonly Vector2[] StartSpawns =
        {
            new Vector2(-1.5f,  1.5f), new Vector2( 1.5f,  0.5f),
            new Vector2(-0.5f, -1.5f), new Vector2( 2.5f, -1.5f),
            new Vector2(-2.5f, -0.5f), new Vector2( 0.5f,  2.5f),
        };
