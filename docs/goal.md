...
_**보류 중 — 활성 목표 없음** (2026-08-06, 운영자 "여기 goal로 세팅된거 일단 꺼줘")._

이 파일은 세션 시작 시 **가장 먼저** 읽히고, 여기 활성 목표가 있으면 에이전트는
그것을 진행하려 한다.  운영자가 그 자동 진행을 잠시 끄기를 요청했다.

`CLAUDE.md` 규약대로, 활성 목표가 비어 있으면 에이전트는 **목표를 지어내지 않고
운영자에게 묻는다.**  즉 이 상태에서는 지시받은 일만 한다.

다시 켜려면 아래 '보류된 목표' 의 블록을 이 절로 되돌리면 된다 — 내용은 지우지
않고 그대로 보관해 두었다.

<!-- suggest 2026-08-12 — 운영자가 /goal 로 이번 세션에 직접 목표를 지시함:
  "범선 항해 게임 프로토타입 제작 - 배+바다 3D로 구현하고 카메라만 2.5D/3D
  전환가능하게 구성. 어느정도 배를 움직일수있게 구성해봐" (주간토큰 50% 상한).
  PawnSim(NAN2026 제출작)과는 무관한 별개 차기작 — 이 파일의 "활성 목표 없음"
  상태와는 다른, 세션 스코프의 직접 지시였다. v0(배 부력·이동·카메라 2종)까지
  완료 — 상세는 docs/roadmap.md Done(2026-08-12) 항목과
  docs/next-title-naval-prototype-2026-08-12.md 참고. 이 goal.md 의 정식
  Active goal 로 승격할지, "차기작" 이라는 별도 트랙으로 둘지는 운영자 판단 —
  에이전트가 임의로 승격하지 않는다(이 파일 자체의 유지 계약).
-->

---

## 보류된 목표 (활성 아님 — 되돌리려면 위 절로 이동)


### 2026-06-12 | PawnSim — "유저가 재미를 느낄 수준"의 콜로니심 프로토타입 (운영자 직접 설정)

**운영자 원문 (2026-06-12 오전, 직접 플레이 후):** "피드백준 내용들을 수정하고
검증해서 최대한 처리하고 → 레퍼런스 콜로니심와 동일한 기본 게임성을 확보를 위해 부족한
부분을 찾아서 개선한다 → 처음에 저장공간을 설정하고 살 집을 짓고 농사 지역을
설정하고 그런 기본부터 충실히 구현한다.  거기서 꼭 필요한게 있다면 추가 구현.
유저가 재미를 느낄 수준의 프로토타입이 되어야한다.  **지금의 수준은 난 게임이
아니라고 생각함.  기본 사이클 조차 제대로 구현되지 않았음.**"

**Done when:**
1. 운영자 피드백 14건(operator-feedback-2026-06-12-am.md) 전부 처리·검증 (grader 평결).
2. '새 게임 첫 사이클' — 저장구역 설정 → 집 건설 → 농사 지역 설정 → 식사/수면 —
   이 레퍼런스(레퍼런스 콜로니심)와 같은 감각으로 돌아간다 (갭 분석 → 백로그 → 집행 → 격리 채점).
3. 운영자가 직접 플레이하고 "게임이다"라고 인정.

작업 방식: WORKFLOW-V2 (재현-우선 + 규칙 7~9 검증 루프).  이전 활성 목표(멀티스킬
프레임워크)는 이 게임 목표의 하위 수단으로 유지.
