...
- **Phase 1~6 전부 완료** — 커밋 `f407490`, 브랜치 `langgraph-migration`
- **두 라인 모두 코드로 이어졌다:**

```
쇼츠  스틸 → 🚪문1 → 🧑승인 → 영상화 → 🚪문2 → 조립 → ⚖️법률 → 출시
게임  PM → 검토 → 코드·아트·사운드 병렬 → 🔒Unity → QA → TA → 병합
```
...
| ✓ | ~~진입 마법사~~ | — | `8c67d7e` `scripts/start-here.sh` |
| ✓ | ~~Phase 6 — 게임 라인~~ | — | `f407490` 뮤텍스·거짓검증·병합 |
| 1 | 게임 라인 실물 1사이클 | Unity 필요 | `--mock` 없이 |
| 2 | 게임 플러그인 등록 · 프로필 로더 | 제품화 때 | 로더는 §5 승인 대상 |
...
| 06 | 게임 라인 (뮤텍스 + 상태 병합) | **완료** | ~70K |

**Phase 6 — 게임 라인이 푸는 문제 3가지**

```
PM 발행 → 3인 동시 검토 → 코드·아트·사운드 병렬
        → 🔒Unity(scene→build) 직렬 → QA → TA → PM 병합
```

1. **뮤텍스.** "Unity batchmode는 단일 배타 자원 → 빌드/검증만 직렬, 제작은 병렬"이
   산문 규칙이었다. 이제 **그래프 위상 자체가 락**이라 동시 빌드가 구조적으로 불가능하다.
2. **거짓 검증 차단.** 빌드 폴더가 날짜 스탬프라 자정을 넘기면 어제 빌드를 열고
   "고쳤다"가 된다. `unity_build`가 산출물 경로를 확정해 상태에 넣고 **QA는 그 경로만
   읽는다** — 날짜 폴더를 재탐색하지 않으므로 stale을 읽을 수 없다. 추가로 이번 실행
   시각 이후에 생긴 폴더만 인정하는 stale guard도 둔다.
3. **상태 병합.** `wb/*.json` 94개를 `json.dumps`가 합친다. 손으로 병합하다 JSON이
   깨진 사고(`6d8d7e9`)가 구조적으로 안 난다. **검증: 94개 병합, 깨짐 0.**

실행: `python -m graph.game_graph run --task "..." --mock`
...
3. **LangGraph 함정 둘.**
   - **State에 선언 안 한 키는 조용히 버려진다.** `clip_gate_open`을 빠뜨려 문 2가
     항상 닫혀 있었다. 에러도 경고도 없다. **스키마 선언이 곧 계약.**
   - **한 노드에 조건부 엣지를 두 번 걸면 서로 덮어쓴다.** 게임 그래프에서 승인 판정과
...
7. **ffmpeg concat 목록은 목록 파일이 있는 디렉터리 기준이다.** 클립은 `clips/`,
   최종본은 `outputs/` 라 목록을 `outputs/`에 두면 못 찾는다.
   **mock은 파일을 안 읽으므로 이 버그를 못 잡는다** — 실물로만 나온다.

> **패턴:** 여기 7건 중 5건(1·2·5·6·7)이 **mock으로는 안 잡히는** 종류였다.
> 배선 검증은 싸지만, 배선이 맞다고 동작이 맞는 건 아니다.
> 각 단계는 반드시 한 번은 실물로 돌려야 한다.
