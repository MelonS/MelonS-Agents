...
<!-- suggest 2026-08-12 — 새 스레드 시작됨, "활성 작업 없음" 문구가 이제 낡았다:
  운영자가 /goal 로 새 세션 목표를 직접 지시함 — "범선 항해 게임 프로토타입 제작,
  배와 바다 우선, 주간토큰 50% 전까지 자율작업". 이건 NAN2026 본선 대비가 아니라
  별개의 차기작(운영자 표현) 착수다 — PawnSim 제출물(site/play/, YouTube, 8/9
  배포본)은 전혀 건드리지 않음, 새 씬/스크립트만 추가(별도 파일).
  v0 완료 — 아래 Done 항목 참조. Now 절을 이 상태에 맞게 갱신할지는 운영자 판단.
-->
...
- **2026-08-12 (차기작 — 범선 항해 게임)** 이동 프로토타입 v0 — `550679b`
  ...
  새 씬/스크립트 — 8/9 배포본 무영향.
  · **바다**: Gerstner wave 3파 합성 셰이더(Y축만, OpenMMO water system 파장
    20/14/9m 참고) + CPU 쪽 동일 공식(`OceanWaveSampler`) — 드리프트 없게 머티리얼
    파라미터를 공유.
  · **배**: `ShipBuoyancy`(4점 샘플 → Y·피치·롤) + `ShipController`(스로틀·방향타,
    관성). `-autopilot` 자동조종 경로로 확인 — pos.z 0→19.9, yaw 0→24° (실제
    키보드 경로는 미검증, 코드 주석에 명시).
  · **카메라**: 2.5D/3D 두 리그 + Tab 전환. 검증 중 실제 버그 1건 발견·수정 —
    `CameraModeSwitcher` 가 `GameObject.SetActive` 만 하고 `Camera.enabled` 는
    안 건드려서 3D 전환 시 검은 화면. 로그 찍어서 원인 확인 후 수정(추측 아님).
  · **라이선스**: 운영자가 쓴 원래 표현이 상용 항해 게임 시리즈(1990년~현재,
    코에이 테크모)의 상표였음을 사용 전 확인, `ip-scrub.py` BANNED 목록에
    추가 + 기존 leak 2건(무관한 곳)도 같이 정리.
  · **다음**: 실제 WASD 플레이 검증(운영자), Tab 전환 실플레이 검증, 3D 배
    모델(현재 박스 placeholder) — 생성형 AI 사용 지시 있었으나 유료 3D 생성
    API는 money firewall 대상이라 미착수, 로컬 대안 조사 필요.

- **2026-08-10 (PawnSim / NAN 2026)** 사전과제 **제출 완료** — `ad1ec0a`.
  신청서 접수 확인(`dl_nan@nhn.com` 메일), ①②③④ 전량 제출, 개인 참가로 ⑤ 생략.
  · **② 시연 영상** https://youtu.be/3iAvYyzoQ3w (공개 · 59.58초 · 소리 있음)
    마을 전경 → 작업 우선순위 설정 → 벌목 지정과 주민 이동 → 각자 다른 작업
    → 방 증축 → 저녁 습격과 방어 → 밤 등불 → 다음 날 아침.
  · **① 플레이** https://melons.github.io/MelonS-Agents/play/ (GET 200 · smoke PASS)
  · **③ 6쪽 / ④ 12쪽** — ④ 는 요강 필수 항목("AI 대상 주요 프롬프트 및 지시 사항")이
    아예 빠져 있던 것을 마감 당일에 발견해 §3 으로 신설했다.
  · **게이트 25/25 PASS** — 마감일에 `p2-defend-raid` 를 새로 추가했다.
    주민에게 전투 행동 자체가 없어 적을 무시하던 버그를 잡은 시나리오다.

  마감일 하루에 고친 것: 전투 행동 신설(주민이 적을 무시하고 하던 일을 계속했다),
  약탈자 체력 20→48(넷이 붙으면 1.25초에 죽었다), 자막 7줄 재작성(주어가 전부
  '주민'이라 자동 게임처럼 읽혔다), 조작 UI 복원(끄면 화면이 저절로 굴러가 보인다),
  버튼을 좌표 시늉이 아니라 실제 onClick 으로 누르게, 증축을 벽 한 줄에서
  닫힌 방으로(지붕 아래 잠자리 6/9 → 8/9).

  **심사 종료까지 유지할 것**: 플레이 링크 · YouTube 영상(공개) · 저장소 공개.
  접수 마감 후 제출 링크는 변경 불가.  다음 일정 — 08-22 참가팀 발표(10팀),
  09-04~06 본선 48시간(판교, 전일 참여 필수).
