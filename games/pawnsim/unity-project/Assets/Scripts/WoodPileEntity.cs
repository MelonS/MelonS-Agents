...
namespace MelonS.GameProto
{
    ...
    public class WoodPileEntity : MonoBehaviour
    {
        ...
        public static float PileScale(int amount)
        {
            ...
            //   높이를 더 올리면 폭이 칸을 넘는다.)
            // 2026-07-31 2차 — 스프라이트 원본을 다시 그려(gen-ground-items.py)
            //  내용이 타일의 0.72~0.88 x 0.59~0.75 가 됐다(이전 0.5x0.25).
            //  원본이 제 크기를 갖췄으므로 스케일로 억지로 키울 필요가 없다 —
            //  0.95~1.35 를 그대로 두면 큰 더미가 한 칸을 넘는다.
            //  0.72~1.0: 작은 더미 0.63x0.42 타일, 만재 0.88x0.59 타일.
            //  사람(0.44x0.91)의 절반 남짓 — 레퍼런스의 바닥 더미 비율에 근접한다.
            return Mathf.Lerp(0.72f, 1.0f, t);
        }
