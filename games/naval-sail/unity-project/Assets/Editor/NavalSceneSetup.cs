...
{
    /// <summary>
    /// 배-바다 이동 프로토타입 씬 생성기. batchmode -executeMethod 로 씬을
    /// 코드에서 재현 가능하게 굽는다 (PawnSim 의 SceneSetup.cs 와 같은 패턴 —
    /// 2026-08-12에 PawnSim 프로젝트에서 이 파일을 그대로 옮겨 왔다, 이 프로젝트
    /// 는 PawnSim과 완전히 분리된 별도 Unity 프로젝트다).
    ///
    /// Invoked from CLI:
    ...
    public static class NavalSceneSetup
    {
        ...
        private static GameObject SetupShip()
        {
            ...
            // firewall 대상이라 안 쓰고, PawnSim 도 쓰던 Kenney CC0 경로를
            // 그대로 따름 — game-artist 에이전트 우선순위(Kenney CC0 우선,
            // SDXL 은 최후수단)와 일치.
