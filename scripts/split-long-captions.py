...
CHAR_MAX tuned for libass Fontsize=44 in a 1080-wide frame with
MarginH=100, leaving ~880 px usable width.  At Korean glyph width
~50 px and English char width ~22 px, ~28 chars fits in one line
for either language.
...
def merge_short_neighbours(items, min_duration: float):
    """items: list of (start, end, text).  Merge any item shorter than
    min_duration into its previous sibling by extending the previous
    item's end and concatenating texts.  If the first item is too
    short, instead extend its end into the next item's start."""
    if not items:
        return items
    merged = [items[0]]
    for s, e, t in items[1:]:
        if e - s < min_duration and merged:
            ps, pe, pt = merged[-1]
...
def main():
    ...
    expanded = merge_short_neighbours(expanded, args.min_duration)
