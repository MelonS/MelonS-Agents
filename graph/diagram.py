...
SHORTS = Layout(
    nodes={
        ...
        "approval": Node("approval · interrupt<br/>자율 모드면 halt + 블로커 기록",
                         "approval · interrupt<br/>autonomous run halts + logs", HUMAN),
        ...
        "legal": Node("legal · legal-gate.sh<br/>미실행 = fail-closed",
                      "legal · legal-gate.sh<br/>not run = fail-closed", GATE),
...
GAME = Layout(
    nodes={
        ...
        "qa": Node("qa<br/>exe 실행 · 스크린샷<br/>★ 상태의 경로만 읽음",
                   "qa<br/>launch exe · screenshot<br/>★ reads pinned paths only"),
