...
from each mission's `metrics.json`.  To regenerate after a new
mission lands: run [`scripts/setup-venv.sh`](scripts/setup-venv.sh)
once (creates `.venv/` and installs matplotlib), then
`.venv/bin/python scripts/generate-charts.py`.  Every second plotted
above is local CPU / GPU time: `whisper.cpp` for transcribe, `ollama`
...
- [x] Per-mission metric charts — [`docs/metrics/per-mission-time.png`](docs/metrics/per-mission-time.png) + [`docs/metrics/throughput-realtime.png`](docs/metrics/throughput-realtime.png), regenerated by `.venv/bin/python scripts/generate-charts.py` (venv via `scripts/setup-venv.sh`)
