...
### 실제 결과물을 런타임 비용 0으로 만들어 내고, 자기가 만든 게임을 직접 플레이해 스스로 검증하는 멀티 에이전트 시스템.
