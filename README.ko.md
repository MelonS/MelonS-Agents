...
![MelonS-Agents — 숫자로 보기: 출력 100+ (미션 타입 7종), 프로덕션 스킬 1개, 쉐이더 23개, 런타임 API 토큰 0개, 22시나리오 커밋 게이트, 서브에이전트 27개, 감사 레이어 3개, MIT](docs/visuals/01-hero-stats-ko.png)

## 제작 구조도 — 쇼츠 · 게임

이 저장소가 돌리는 두 라인입니다. 각 카드는 그 라인을 실제로 돌리는 LangGraph 상태 기계에서
렌더하므로, 그림을 그린 것이 아니라 **실행된 배선**을 보여 줍니다. 문이 왜 그 자리에 서는지에
대한 실측치는 아래 *파이프라인은 그래프다* 절에 있습니다.

### 쇼츠 제작 구조도

![쇼츠 제작 흐름 — 다크 카드: 26컷 한 편에서 영상 생성이 179분(전체의 81%)을 차지한다는 비용 막대와 스틸 직후의 검사 지점 표시, 이어서 촬영 목록 → 스틸 라운드(장당 9초) → 1차 검사 → 사람 확인 → 컷 라운드(컷당 412초) → 2차 검사 → 합치고 법률 검토 → 출시 준비 완료 흐름, 그리고 다시 돌아가는 길 4개와 중단·재개 안내](docs/visuals/15-graph-shorts-ko.png)

*쇼츠 한 편은 약 3시간이고 그 중 81%가 영상화 단계입니다. 그래서 **문 1**이 기준을 넘지 못한
스틸에 그 시간을 쓰지 못하게 막습니다.*
...
*같은 fan-out 이지만 병렬 레인이 문이 아니라 Unity **뮤텍스**에서 합류합니다. 병목이 다르면
모양도 달라집니다.*
...
| **PawnSim** · `game-dev-agent` 가 제작 | 자기 검증형 콜로니심 게임 프로토타입 | 개발 중 | ✅ [브라우저에서 바로 플레이](https://melons.github.io/MelonS-Agents/play/) — 직접 빌드하려면 Windows + Unity 6000.0.75f1 |
| **content-shorts** · 4팀 법률 검수 파이프라인이 제작 | 주제 → 출처를 갖추고 저작권 검수를 거친 9:16 쇼츠 (정보/뉴스/아이돌 포맷) | 개발 중 | ⚠️ 2026-07-01 첫 쇼츠 출시(아이돌), 이후 정보·뉴스·아이돌 포맷을 계속 렌더, 다만 아직 고정 주기는 아님, Pexels 키 필요 |
...
![검증 — 두 게이트: 커밋마다 22개 시나리오 입력 단위 재현 게이트 + 장시간 소크에 격리 채점 서브에이전트](docs/visuals/14-verification-loop-ko.png)

에이전트에게 코드를 *만들어 내게* 하는 건 누구나 합니다. 어려운 건 그 결과가 실제로 작동한다는 걸 *증명*하는 일이고, 그게 이 프로젝트의 핵심입니다. PawnSim 은 코드가 반영되기 전에 두 개의 게이트를 통과합니다.

- **커밋마다 도는 22개 시나리오 재현 게이트.**  에이전트가 플레이어와 똑같은 UI 경로로 실제 클릭을 합성하고, 각 클릭이 (단지 닿은 게 아니라) 실제로 *효과*를 냈는지, 가령 "그 클릭이 지정(designation)을 정말 찍었는지"를 어서트합니다.
...
이 시스템은 모든 스킬을 하나의 형태로 강제하지 않습니다. **Shape A** 는 5에이전트 미션 파이프라인(orchestrator + planner / resourcer / editor / qa)으로 흐르고, **Shape B** 는 planner/qa 단계가 거의 빌 때 쓰는 독립 스크립트입니다. 서브에이전트끼리는 대화 기록을 공유하지 않고, 커밋된 파일(`plan.md` / `MANIFEST.md` / `qa-report.md`)로만 작업을 넘겨줍니다. 그래서 각자의 컨텍스트와 비용이 일정 범위 안에 묶입니다. 역할별 모델 배정(planner/resourcer = opus, editor/qa = sonnet)과 비용 방화벽 덕분에 Anthropic 토큰은 오케스트레이션 단계에서만 쓰이고, 미션 실행은 전부 로컬 도구로 돌아 런타임 API 토큰이 **0**으로 유지됩니다. `.claude/agents/` 에는 정의가 **27개** 있습니다(코어 6 + 게임 로스터 13, 역할 12종에 아트 QA 게이트 `ta` 를 더한 것 + 콘텐츠 파이프라인 팀 5 + 기획·스틸·완성 컷을 각각 채점하는 심사위원 3). 전체 데이터 흐름도: [`docs/architecture.md`](docs/architecture.md) · 게임 프로토타입 빌드 체인: [`skills/game-dev-agent/ARCHITECTURE.md`](skills/game-dev-agent/ARCHITECTURE.md).
